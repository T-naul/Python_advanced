...
-- SELECT employee_id
-- FROM employees
-- WHERE department_id in (SELECT department_id FROM department WHERE location_id = 1600 OR location_id = 1700));
...
-- FROM employees
-- JOIN (SELECT MIN(salary) as min_salary, job_id FROM employees GROUP BY job_id) as b on employees.salary = b.min_salary AND employees.job_id = b.job_id;
